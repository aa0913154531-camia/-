...
const portfolioItems = [
  ...
  { id: 8, title: "紫水晶洞", desc: "天然紫晶洞，福祿雙全，居家鎮宅必備", img: "https://i.postimg.cc/3wK1L6Jv/LINE-ALBUM-shui-jing-bai-jian-260415-1-0.jpg", link: "https://lin.ee/7a9zKtJ" },
  ...
  return (
    <section className="py-32 bg-[#0a0a0a] relative overflow-hidden text-white">
      <div className="container mx-auto px-8">
        <h2 className="font-serif text-6xl font-bold mb-12 text-[#D4AF37]">精選作品</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
          {portfolioItems.map((item, index) => (
            <motion.div key={item.id} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
              <Card className="bg-[#1a1a1a] border-[#D4AF37]/20 overflow-hidden">
                ...
                <CardContent className="p-8">
                  ...
                  <p className="text-[#D4AF37]/70 mb-8 h-12 overflow-hidden">{item.desc}</p>
                  <button onClick={() => window.open(item.link, '_blank')} className="w-full bg-[#D4AF37] text-black py-3 font-bold">立即下單購買</button>
                </CardContent>
              </Card>
            </motion.div>
          ))}
